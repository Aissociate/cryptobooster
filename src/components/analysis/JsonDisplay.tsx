        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-4">
            <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/30">
              ...
              <div className="text-white font-mono text-lg font-bold">{jsonData.Crypto || ''}</div>
            </div>
            
            <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/30">
              <div className="text-gray-400 text-xs uppercase tracking-wide mb-1">Score</div>
              <div className="text-yellow-400 font-mono text-lg font-bold">{jsonData.score || ''}</div>
            </div>
            
            <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/30">
              <div className="text-gray-400 text-xs uppercase tracking-wide mb-1">Confiance</div>
              <div className="text-emerald-400 font-mono text-lg font-bold">{jsonData.confiance || ''}</div>
            </div>
            ...
            <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/30">
              ...
              <div className={`font-mono text-lg font-bold flex items-center gap-1 ${
                jsonData.direction === 'LONG' ? 'text-emerald-400' : 
                jsonData.direction === 'SHORT' ? 'text-red-400' : 'text-gray-400'
              }`}>
                {jsonData.direction === 'LONG' && <TrendingUp className="w-4 h-4" />}
                {jsonData.direction === 'SHORT' && <TrendingDown className="w-4 h-4" />}
                {jsonData.direction || ''}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-blue-500/10 p-3 rounded-lg border border-blue-500/20">
              ...
              <div className="text-white font-mono text-lg">{jsonData.entree ? `$${jsonData.entree}` : ''}</div>
              {jsonData.date_entree && (
                <div className="text-gray-400 text-xs mt-1">{jsonData.date_entree}</div>
              )}
            </div>
            
            <div className="bg-red-500/10 p-3 rounded-lg border border-red-500/20">
              <div className="text-red-400 text-xs uppercase tracking-wide mb-1">Stop Loss</div>
              <div className="text-white font-mono text-lg">{jsonData.sl ? `$${jsonData.sl}` : ''}</div>
              {jsonData.date_sl && (
                <div className="text-gray-400 text-xs mt-1">{jsonData.date_sl}</div>
              )}
            </div>
            
            <div className="bg-emerald-500/10 p-3 rounded-lg border border-emerald-500/20">
              <div className="text-emerald-400 text-xs uppercase tracking-wide mb-1">TP1</div>
              <div className="text-white font-mono text-lg">{jsonData.tp1 ? `$${jsonData.tp1}` : ''}</div>
              {jsonData.date_tp1 && (
                <div className="text-gray-400 text-xs mt-1">{jsonData.date_tp1}</div>
              )}
            </div>
            
            <div className="bg-emerald-500/10 p-3 rounded-lg border border-emerald-500/20">
              <div className="text-emerald-400 text-xs uppercase tracking-wide mb-1">TP2</div>
              <div className="text-white font-mono text-lg">{jsonData.tp2 ? `$${jsonData.tp2}` : ''}</div>
              {jsonData.date_tp2 && (
                <div className="text-gray-400 text-xs mt-1">{jsonData.date_tp2}</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div className="bg-emerald-500/10 p-3 rounded-lg border border-emerald-500/20">
              ...
              <div className="text-white font-mono text-xl">{jsonData.score_bullish || ''}</div>
            </div>
            
            <div className="bg-red-500/10 p-3 rounded-lg border border-red-500/20">
              <div className="text-red-400 text-sm font-medium mb-1">Score Bearish</div>
              <div className="text-white font-mono text-xl">{jsonData.score_bearish || ''}</div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            {jsonData.timeframes_dominants && (
              <div className="bg-gray-900/40 p-3 rounded-lg border border-gray-600/20">
                <div className="text-blue-400 text-sm font-medium mb-1">Timeframes Dominants</div>
                <div className="text-gray-200">
                  {Array.isArray(jsonData.timeframes_dominants) 
                    ? jsonData.timeframes_dominants.join(', ') 
                    : jsonData.timeframes_dominants
                  }
                </div>
              </div>
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-red-500/10 p-3 rounded-lg border border-red-500/20">
              ...
              <div className="text-white font-mono text-lg">{jsonData.support_principal ? `$${jsonData.support_principal}` : ''}</div>
            </div>
            
            <div className="bg-emerald-500/10 p-3 rounded-lg border border-emerald-500/20">
              <div className="text-emerald-400 text-xs uppercase tracking-wide mb-1">Résistance Principale</div>
              <div className="text-white font-mono text-lg">{jsonData.resistance_principale ? `$${jsonData.resistance_principale}` : ''}</div>
            </div>
            ...
            <div className="bg-red-400/10 p-3 rounded-lg border border-red-400/20">
              ...
              <div className="text-white font-mono text-lg">{jsonData.support_secondaire ? `$${jsonData.support_secondaire}` : ''}</div>
            </div>
            ...
            <div className="bg-emerald-400/10 p-3 rounded-lg border border-emerald-400/20">
              ...
              <div className="text-white font-mono text-lg">{jsonData.resistance_secondaire ? `$${jsonData.resistance_secondaire}` : ''}</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <div className="text-gray-300 font-mono text-xs mt-1">
                {jsonData.generated_at ? 
                  new Date(jsonData.generated_at).toLocaleString('fr-FR') :
            ...
            <div>
              ...
              {!(jsonData.metadata?.is_verified || jsonData.crypto_info?.is_verified) && jsonData.metadata && (
                <span className="ml-2 text-xs px-2 py-1 rounded-full bg-gray-500/20 text-gray-400">
                  ○ En attente
                </span>
              )}
            </div>
